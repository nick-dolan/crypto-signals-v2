...
Для графика `1h` TradingView автоматически использует `1m` intrabars. Живые тесты по BTC, ETH и SOL вернули 1 668 часовых строк, но первые две содержат `null` во всех полезных полях из-за прогрева study. Фактически доступны 1 666 полных числовых часов — около 69,4 дня. Более крупный intrabar увеличивает доступную глубину истории ценой меньшей точности delta.

Запрос большего числа баров не даёт больше данных: заполненными остаются всё те же 1 666 часов, а более старые строки приходят пустыми. Поэтому для Volume Delta используем последние 1 666 непрерывных числовых часов.

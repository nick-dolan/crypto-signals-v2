...
function createCandidateRow (profile) {
  ...
  return [
    ...
    roundNullable(sustained?.history_hours ?? null),
    sustained?.peer_count ?? null,
    sustained?.down_windows ?? null,
    sustained?.up_windows ?? null,
    sustained?.daily_windows ?? null,
    sustained?.weekly_windows ?? null,
    ...
    roundScaledNullable(sustained?.up_excess_median ?? null, 100),
    roundNullable(sustained?.daily_win_rate ?? null),
    roundNullable(sustained?.weekly_win_rate ?? null),
    roundScaledNullable(sustained?.excess_4h ?? null, 100),
    roundScaledNullable(sustained?.excess_12h ?? null, 100),
    roundScaledNullable(sustained?.excess_24h ?? null, 100),
    roundScaledNullable(sustained?.excess_7d ?? null, 100),
...
export function buildAgentPayload (shortlist) {
  ...
  const payload = {
    ...
    conventions: {
      ...
      sustainedStrength: "Контекст относительной силы, рассчитанный на шаге 4 до предварительного отбора. Peers — другие монеты всей вселенной, сама монета исключена; минимум 3 peers. Используется вся доступная OHLCV-история: непересекающиеся окна каждой длины отсчитываются назад от asOf, самое старое неполное окно отброшено. Окно с невалидной ценой любой монеты исключается для всей вселенной, состав peers не меняется. Сравнения simple return учитывают допуск 1e-12 в исходной шкале. sustainedStatus рассчитан до округления: не пересчитывай его по округлённым полям. Scores 0–100 — эвристики, не вероятности события",
    ...
    schema: [
      ...
      "sustainedHistoryHours",
      "sustainedPeerCount",
      "sustainedDownWindows",
      "sustainedUpWindows",
      "sustainedDailyWindows",
      "sustainedWeeklyWindows",
      "sustainedDownWinRate",
      "sustainedDownPositiveRate",
      "sustainedDownExcessMedianPct",
      "sustainedUpParticipationRate",
      "sustainedUpExcessMedianPct",
      "sustainedDailyWinRate",
      "sustainedWeeklyWinRate",
      "sustainedExcess4hPct",
      "sustainedExcess12hPct",
      "sustainedExcess24hPct",
      "sustainedExcess7dPct",
      "categoryMoveAtr",
      "categoryBreadth",
      "coinLeadAtr",
      "flags",
    ...
    definitions: {
      ...
      sustainedStatus: "Context: persistent — историческая и текущая сила при sustainedExcess7dPct > 0; иначе emerging при текущей силе; fading — историческая сила без текущей; neutral — ни то ни другое. Историческая сила: sustainedHistoryScore >= 65, sustainedDownWinRate >= 0.6, sustainedUpParticipationRate >= 0.5, sustainedDailyWinRate и sustainedWeeklyWinRate >= 0.55. Текущая сила: sustainedCurrentScore >= 65 и sustainedExcess4hPct, sustainedExcess12hPct, sustainedExcess24hPct > 0. Если любой score недоступен или блок отсутствует, insufficient_data имеет приоритет. Не сигнал входа и не прогноз направления",
      sustainedHistoryScore: "Context: историческая эвристика 0–100 = 100 × среднее sustainedDownWinRate, sustainedUpParticipationRate, sustainedDailyWinRate и sustainedWeeklyWinRate; не вероятность. Доступна при >= 28 суточных, >= 4 недельных, >= 12 медвежьих и >= 12 бычьих окон",
      ...
      sustainedHistoryHours: "Context: длительность доступной OHLCV-истории в часах между первым и последним close, не число свечей",
      sustainedPeerCount: "Context: число других монет вселенной для сравнения до предварительного отбора, не число кандидатов",
      sustainedDownWindows: "Context: число доступных непересекающихся медвежьих окон 4h: строго > 55% peers падают и TOTAL3ES снижается",
      sustainedUpWindows: "Context: число доступных непересекающихся бычьих окон 4h: строго > 55% peers растут и TOTAL3ES растёт",
      sustainedDailyWindows: "Context: число доступных непересекающихся окон 24h для сравнения с медианой peers",
      sustainedWeeklyWindows: "Context: число доступных непересекающихся окон 168h для сравнения с медианой peers",
      ...
      sustainedUpExcessMedianPct: "Context: медиана разницы simple return монеты и медианы peers на бычьих окнах 4h, п.п. (исходная разница × 100)",
      sustainedDailyWinRate: "Context: доля непересекающихся окон 24h, когда simple return монеты строго выше медианы peers, 0–1",
      sustainedWeeklyWinRate: "Context: доля непересекающихся окон 168h, когда simple return монеты строго выше медианы peers, 0–1",
      sustainedExcess4hPct: "Context: simple return монеты минус медиана simple return peers за последние 4h, п.п. (исходная разница × 100)",
      sustainedExcess12hPct: "Context: simple return монеты минус медиана simple return peers за последние 12h, п.п. (исходная разница × 100)",
      ...
      sustainedExcess7dPct: "Context: simple return монеты минус медиана simple return peers за последние 168h, п.п. (исходная разница × 100)",

...
import { renderReportHtml } from "../src/steps/step7.1-report/render-report-html.js"
...
test("agent text cannot escape embedded JSON, become executable HTML, or replace template slots", async () => {
  ...
  const report = {
    coins: [{ symbol: unsafe, explanation: unsafe, drivers: [unsafe], history: { warning: unsafe } }],
    definitions: { unsafe },
  }
  const html = await renderReportHtml(report)
  const embedded = scripts(html)
